...
with tf.compat.v1.variable_scope('layer_1'):
    weights = tf.compat.v1.get_variable("weights1", shape=[number_of_inputs, layer_1_nodes], initializer=tf.keras.initializers.glorot_normal())
    biases = tf.compat.v1.get_variable(name="biases1", shape=[layer_1_nodes], initializer=tf.zeros_initializer())
    layer_1_output = tf.compat.v1.nn.relu(tf.matmul(X, weights) + biases)

# Layer 2
with tf.compat.v1.variable_scope('layer_2'):
    weights = tf.compat.v1.get_variable("weights2", shape=[layer_1_nodes, layer_2_nodes], initializer=tf.keras.initializers.glorot_normal())
...
with tf.compat.v1.variable_scope('layer_3'):
    weights = tf.compat.v1.get_variable("weights3", shape=[layer_2_nodes, layer_3_nodes], initializer=tf.keras.initializers.glorot_normal())
    biases = tf.compat.v1.get_variable(name="biases3", shape=[layer_3_nodes], initializer=tf.zeros_initializer())
    layer_3_output = tf.nn.relu(tf.matmul(layer_2_output, weights) + biases)

# Output Layer
with tf.compat.v1.variable_scope('output'):
    weights = tf.compat.v1.get_variable("weights4", shape=[layer_3_nodes, number_of_outputs], initializer=tf.keras.initializers.glorot_normal())
